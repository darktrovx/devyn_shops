...
local function addItemToSecondSlot(id, price, payload)
    SetTimeout(50, function()
        local addMoney = ox_inventory:AddItem(id, 'money_dirty', price, nil, 1)
...
local function resetInventory(source, id)
    SetTimeout(100, function()
        ...
        lib.logger(source, ('shops_stashitems:%s'):format(id), items)
        TriggerEvent('shops:server:soldStashItems', source, id, items)
    end)
end
...
local function createSaleStash(id, label, items, coords)
    id = ('salesStash_%s'):format(id)

    if inventories[id] then return end
...
ox_inventory:registerHook('swapItems', function(payload)
    ...
    if payload.toInventory == payload.fromInventory then return false end

    if item == 'money_dirty' and not addItem then
    ...
    if not price or openedBy[inventory] ~= source then return false end
    ...
    if addItem then
        if payload.toSlot == 1 then
            ...
            local added = ox_inventory:AddItem(inventory, 'money_dirty', price, nil, 1)
    ...
    elseif payload.fromSlot.slot > 1 and payload.fromSlot.slot == slotCount-1 then
        local removed = ox_inventory:RemoveItem(inventory, 'money_dirty', price, nil, 1)
